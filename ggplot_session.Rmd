...
Error bar plot with values from tidyverse.

```{r, echo = T, tidy = T}
g <- ggplot(data = iris2, aes(x = Species, y = mean, fill = Species)) + 
  geom_bar(stat = "identity") +
...
```{r, echo = F, fig.cap="speed comparison between summarySE() and the tidy approach."}
